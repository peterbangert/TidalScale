...
//                1000000000L
    ...
    public static class RandomOperation implements MapFunction<PersonContact, PersonContact> {

        @Override
        public PersonContact map(PersonContact personContact) throws Exception {
            float x = 0;
            for(int i=0; i<=100; i++) {
                x += personContact.getPerson1().doubleValue() + personContact.getPerson2().doubleValue();
            }
            Long p1 = personContact.getPerson1();
            Long p2 = personContact.getPerson2();
            if(x > 101*(personContact.getPerson1().doubleValue() + personContact.getPerson2().doubleValue())){
                return new PersonContact(personContact.eventType(), personContact.getUuid(), personContact.getSequenceNumber(), p2, p1, personContact.getOccurredOn(), personContact.getCity());
            }
    ...
    public void highRiskContactProducer() throws Exception {
        ...
        DataStream<PersonContact> joinStream = contactStream.map(new RandomOperation());

        org.apache.flink.streaming.api.windowing.time.Time windowTime = org.apache.flink.streaming.api.windowing.time.Time.milliseconds(1);

        DataStream<PersonContact> joinedContactStream = contactStream.join(joinStream)
                .where(new KeySelector<PersonContact, Long>() {
                    @Override
                    public Long getKey(PersonContact personContact) throws Exception {
                        return personContact.getPerson1();
                    }
                }).equalTo(new KeySelector<PersonContact, Long>() {
            @Override
            public Long getKey(PersonContact personContact) throws Exception {
                return personContact.getPerson2();
            }
        }).window(TumblingEventTimeWindows.of(
                        windowTime
                )).apply(new JoinFunction<PersonContact, PersonContact, PersonContact>() {
            @Override
            public PersonContact join(PersonContact personContact, PersonContact personContact2) throws Exception {
                return personContact;
            }
        });

        joinedContactStream.filter(personContact -> {
            return personContact.getPerson1().longValue() >= Long.MAX_VALUE;
        }).map(personContact -> {
            return Long.toString(personContact.getPerson1().longValue());
