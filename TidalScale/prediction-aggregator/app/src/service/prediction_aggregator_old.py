...
class PredictionAggregator:
    ...
    def update_prediction(self, prediction_data):
        ...
        if lt_prediction != 0:
            ...
            prev_timestamp = self.lt_history[-1][0]
            prev_prediction = self.lt_history[-1][1]
            ...
            self.lt_history += interp_predictions
            self.lt_history = self.append_and_prune(lt_horizon, lt_prediction, self.lt_history)
        ...
        self.linear = True
        self.st_score, st_prediction = self.calculate_score(st_prediction, self.st_history, self.trace_history)
        self.lt_score, lt_prediction = self.calculate_score(lt_prediction, self.lt_history, self.trace_history)
        self.st_weight, self.lt_weight = self.calculate_weight(self.st_score, self.lt_score)
