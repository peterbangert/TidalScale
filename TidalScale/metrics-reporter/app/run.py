...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--prometheus', help='<Address:Port> Prometheus IP Adress')
    ...
    metric_topic = config.kafka['metric_topic'] if not args.local else config.kafka_local['metric_topic']
    metric_topic_partitions = config.kafka['metric_topic_partitions'] if not args.local else config.kafka_local['metric_topic_partitions']
    if not kafka_utils.check_topic_exists(bootstrap_server, metric_topic):
        kafka_utils.create_topic(bootstrap_server, metric_topic, metric_topic_partitions)
